...
class Map():
  ...
  def generate_paths(self,startNode,endNode,pathDict,validPaths,currentPath =[]):
    ...
    # print(os.linesep)
    is_repeat_visit_small_cave = any([el for el in currentPath if str.islower(el) and currentPath.count(el)>1])

    for node in pathDict[startNode]:
      if str.islower(node) and (node in currentPath) and is_repeat_visit_small_cave:
        continue
      newPathDict = pathDict
      newCurrentPath = currentPath.copy()
      if str.islower(startNode) and  is_repeat_visit_small_cave:
